...
namespace XJT.Com.EntitySql.Common
{
    public class NPOIHelper
    {
        ...
        /// </summary>
        /// <param name="stream">Excel流</param>
        /// <param name="ignoreSheet">忽略的sheet名称</param>
        /// <returns>Excel中的数据</returns>
        public static DataSet ImportExceltoDs(Stream stream, IEnumerable<string> ignoreSheet)
        {
            try
            {
                DataSet ds = new DataSet();
                ...
                {
                    ISheet sheet = wb.GetSheetAt(i);
                    if (!ignoreSheet.Contains(sheet.SheetName))
                    {
                        DataTable dt = new DataTable(sheet.SheetName);
                        dt = ImportDt(sheet, 0, true);
                        ds.Tables.Add(dt);
        ...
        /// <summary>
        /// DataTable导出到Excel的MemoryStream第二步
        /// </summary>
        /// <param name="dtSource">源DataTable</param>
        /// <param name="strHeaderText">表头文本</param>
